...
def cursor_git_dir(workspace: Path) -> Path:
    marker = workspace / ".git"
    if marker.is_dir():
        return marker.resolve()
    if not marker.is_file():
        raise DispatchRefused(f"Cursor workspace has no .git marker: {workspace}")
    declaration = marker.read_text(encoding="utf-8", errors="strict").strip()
    prefix = "gitdir: "
    if not declaration.startswith(prefix):
        raise DispatchRefused(f"Cursor workspace has an invalid .git marker: {workspace}")
    git_dir = Path(declaration[len(prefix) :])
    if not git_dir.is_absolute():
        git_dir = marker.parent / git_dir
    git_dir = git_dir.resolve()
    if not git_dir.is_dir():
        raise DispatchRefused(f"Cursor git directory does not exist: {git_dir}")
    return git_dir


def require_current_workspace(workspace: Path) -> None:
    if workspace.resolve() != ROOT.resolve():
        raise DispatchRefused(
            f"Gate B is closed; working directory must be this checkout: {ROOT.resolve()}"
        )
...
def build_command(probe: Probe, brief_path: Path, cwd: Path) -> list[str]:
    ...
    if harness.name == "cursor-composer":
        ...
        return [
            probe.executable,
            *harness.invocation[1:-3],
            to_wsl_path(cwd),
            brief_prompt,
            to_wsl_path(cursor_git_dir(cwd)),
        ]
