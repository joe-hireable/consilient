...
# Load run_exp45 module from path
_exp45_path = Path(__file__).resolve().parent.parent / "docs" / "10-research" / "experiments" / "exp45" / "run_exp45.py"
_spec = importlib.util.spec_from_file_location("run_exp45", _exp45_path)
if _spec is None or _spec.loader is None:
    raise ImportError(f"Could not load run_exp45 from {_exp45_path}")
_mod = importlib.util.module_from_spec(_spec)
sys.modules["run_exp45"] = _mod
_spec.loader.exec_module(_mod)

BoundaryEvent = _mod.BoundaryEvent
analyze_boundary_retention = _mod.analyze_boundary_retention
analyze_transcript_file = _mod.analyze_transcript_file
bootstrap_ci = _mod.bootstrap_ci
extract_record_entities = _mod.extract_record_entities
normalize_path = _mod.normalize_path
percentile = _mod.percentile
point_biserial_r = _mod.point_biserial_r
rank_data = _mod.rank_data
run_exp45_analysis = _mod.run_exp45_analysis
