...
def test_fan_out_runs_and_returns_both_results(
    ...
    monkeypatch.setattr(SCRIPT_MODULE, "ROOT", tmp_path)
...
def test_cursor_shell_receives_workspace_and_task_as_arguments() -> None:
    ...
    assert '"GIT_DIR=$3"' in cursor.invocation[4]
    assert '"GIT_WORK_TREE=$1"' in cursor.invocation[4]
    assert "git rev-parse --show-toplevel" in cursor.invocation[4]
    assert "CURSOR_API_KEY" not in cursor.invocation[4]
    assert "CURSOR_API_ENDPOINT" not in cursor.invocation[4]
    assert cursor.invocation[-3:] == ("{cwd}", "{brief}", "{git_dir}")
...
def test_cursor_command_passes_an_absolute_brief_outside_shell_text(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    cursor = next(harness for harness in REGISTRY if harness.name == "cursor-composer")
    ...
    workspace = Path("C:/work tree/repository")
    monkeypatch.setattr(
        SCRIPT_MODULE,
        "cursor_git_dir",
        lambda path: Path("C:/git/worktrees/repository"),
    )
    command = SCRIPT_MODULE.build_command(probe, brief, workspace)
    ...
    assert command[-3:] == [
        ...
        "/mnt/c/git/worktrees/repository",
    ]


def test_cursor_translates_the_linked_worktree_git_directory(tmp_path: Path) -> None:
    ...
    assert SCRIPT_MODULE.cursor_git_dir(workspace) == metadata.resolve()


def test_gate_b_refuses_a_foreign_working_directory_before_probe(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "root"
    ...
    def unexpected_probe() -> None:
        raise AssertionError("foreign workspace reached the execution boundary")

    monkeypatch.setattr(SCRIPT_MODULE, "ROOT", root)
    monkeypatch.setattr(SCRIPT_MODULE, "probe_registry", unexpected_probe)
    monkeypatch.setattr(
        SCRIPT_MODULE,
        "_record_refusal",
        lambda task, cwd, reason: refusals.append(reason),
    )

    code = SCRIPT_MODULE.main(
        ["--supervised", "--cwd", str(foreign), "foreign task"]
    )

    assert code == 2
    assert refusals and "Gate B is closed" in refusals[0]
    assert "Gate B is closed" in capsys.readouterr().err
