"""Which rows may enter β, and the interval computed once they have.

Split from `beta.py` on 28 August 2026 by dependency layer rather than by subject, so
that nothing here reaches back into the module it came from. That is not a filing
convenience: it puts the admission rules underneath every caller of them, where a rule
cannot be bypassed by import order.

The division follows the defect this meter was built around. `beta.py`'s own record puts
it plainly -- "the arithmetic in `compute` is correct; the exposure is entirely in which
rows exist" -- so admission is the subject, and it is what lives here:
`admits_human_beta_row`, `admits_sizing_input`, `admits_upstream_maintainer_proxy_row`,
`refuse_upstream_row`, `counts_toward_upstream_beta_numerator` and
`verifier_verdict_did_not_gate_submission`, with the thresholds (`MIN_REJECTIONS`,
`MIN_COMPOSITE_REJECTION_FRACTION`) and the estimator (`Beta`, `compute`, `wilson`) they
gate.

A row admitted here is a row the figure will carry, so each predicate refuses rather
than coerces. `beta.py` keeps the two entry points that read a live store and re-exports
every name that could be imported from it before the split; `__all__` says which.

GOVERNED FILE. `promote.PROTECTED_PREFIXES` matches by `startswith` on the full path, so
`beta_admission.py` does not inherit `beta.py`'s protection by resembling it -- it is
listed there explicitly. Without that entry the self-editing promoter would hold write
access to the admission rules for its own β, which is the shape of failure this
...
def upstream_human_verdict_for_beta(
    row: dict[str, object],
...
def both_upstream_cells_representable(rows: Iterable[dict[str, object]]) -> bool:
    ...
    for row in rows:
        if refuse_upstream_row(row) is not None:
            continue
        if (
            row.get("verifier_accept") is True
            and upstream_human_verdict_for_beta(row) == "reject"
        ):
...
def composite_verifier_rejection_fraction(
    ...
    admitted = [
        ...
        and (
            verifier_version is None or row.get("verifier_version") == verifier_version
        )
