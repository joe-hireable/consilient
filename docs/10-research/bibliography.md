...
| [FULL] | **Anthropic**, [Claude Code status line](https://code.claude.com/docs/en/statusline), [slash commands](https://code.claude.com/docs/en/commands) and [cost management](https://code.claude.com/docs/en/costs). Read 2026-08-19. The status-line JSON includes five-hour and seven-day rate-limit utilisation and reset times, but it refreshes after a response and can be stale across concurrent sessions. `/usage` is the human-facing plan view; `--max-budget-usd` applies to API-billed use, not subscription headroom. |
| [FULL] | **OpenAI**, [Codex app-server protocol](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md). Read and queried locally 2026-08-19. `account/rateLimits/read` returns provider windows with used percentage, reset time and duration; the ordinary top-level CLI help exposes no equivalent quota command. |
