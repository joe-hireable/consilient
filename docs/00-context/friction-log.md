...
| 2026-08-22 | Hand-wrote a terminal `dispatch.outcome` event twice to release a claim held by a dispatch I had killed, because abandonment is not detected and the claim otherwise sits until timeout plus grace — up to an hour — blocking any re-route of the same brief | twice in one evening, once per stalled arm | claim release on abandonment, not on expiry (ADR-0072 closure path) |
| 2026-08-22 | Truncated eighteen 40-hex digests in a document by hand because the leak detector resolves them as commits only, so our own working-tree blob digests read as foreign identifiers | once, but recurs whenever an audit pins the artefacts it examined | teach `check_foreign_identifiers.py` to resolve blob and tree objects, not commits alone |
| 2026-08-22 | Added six missing rows to `docs/decisions/index.md` by hand after instructing every parallel stream not to touch it, which prevented collisions and produced drift instead | once per burst of parallel ADR authoring | L01, the generated ADR index (ADR-0073) |
| 2026-08-23 | Copied untracked `.harness` instance files into a fresh worktree by hand so the suite could run there; without them ten tests failed on `FileNotFoundError` and looked like merge damage | every new worktree | a documented instance-state bootstrap, or tests that skip cleanly on absent instance state |
| 2026-08-23 | Resolved two merge conflicts by hand in `bibliography.md` and `experiment-register.md`, both append-only records where the correct answer was always the union of both sides | every merge touching an append-only record | a union merge driver declared in `.gitattributes` for append-only records |
| 2026-08-23 | Noticed by hand that publishing would delete 586 `weakest_guards` records, because a newer EXP-47 run with seven times the sample stopped populating a field the older run had, and archived the older run rather than lose it | once, but nothing would have caught it | a check that a re-run of an experiment does not empty a field its predecessor populated |
| 2026-08-25 | Set `git config --global core.longpaths true` by hand after finding every review had been dying at clone: the nested workspace path plus this repo's descriptive ADR filenames exceeds Windows MAX_PATH, so the clone succeeded and the checkout failed, and a review that never started looked identical to one that found nothing | once, but it had silently produced zero verdicts for the whole build | a preflight that asserts the longest tracked path plus the workspace prefix fits MAX_PATH, and fails the dispatch loudly rather than the checkout quietly |
| 2026-08-25 | Tried to stop ~7 stuck dispatchers holding a 48 MB trajectory open and found there is no command to do it — no stop marker the dispatcher honours, no kill script, nothing but the OS process list | every time work has to be halted | the kill command Joe asked for on 24 Aug: one instruction that stops every active process immediately, with the claims released rather than left to expire |
| 2026-08-25 | Excised a private-corpus blob from 117 unpushed commits by hand — clone, `filter-branch`, re-audit every introduced blob, reset local — because the leak gate scans the tracked TREE and the thing being published is the HISTORY | once, and it was found only because the gate was run manually before a push | a leak gate that scans every blob introduced since the publish base, not just HEAD's tree, wired into the publication path |
| 2026-08-25 | Restored `.harness/build_driver.py` and `getting-started.md` by hand after the driver left conflict markers in the main worktree and a guard-mutation test left its mutation string in tracked documentation | twice in one night | a post-tick assertion that the main worktree is clean and parses, refusing to continue while it is not |
| 2026-08-25 | Untracked `driver-state.json`, a lock file and the loop's stdout by hand after finding the driver deferring every merge with "1 path(s) dirty in the main tree" — the file it rewrites each tick was itself a tracked git object | once, but it had blocked merging indefinitely | unit Z04's check: `git ls-files .harness` must return no mutable state, no lock and no generated output |
