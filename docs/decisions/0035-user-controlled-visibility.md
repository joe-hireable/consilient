...
The only manipulation known to actually reduce over-reliance is making verification genuinely cheap: over-reliance fell from ~69% to **0%** when a salient display surfaced the error itself — and that display was an **error detector**, not an explanation, whereas written explanations moved it from ~68% to ~66% and higher pay moved it from ~58% to ~57%. `[cited]` — Vasconcelos et al. (CSCW 2023, arXiv:2212.06823). **Show state, not narrative.** That single finding decides the shape of everything below.
...
- `[cited]` Over-reliance tracks verification cost, not information volume: ~69%→**0%** when a display surfaced the error (Study 3, N=286, exploratory); ~68%→~66% for written explanations; ~58%→~57% for higher pay. — Vasconcelos et al. (2023), CSCW, arXiv:2212.06823. **Corrected 2026-08-20 from ~28%**, which conflicted with this repository's own later full-read entry quoting the paper's "average overreliance rate of 0%". The manipulation was an error *detector* rendered in the UI — a verifier — not an explanation.
