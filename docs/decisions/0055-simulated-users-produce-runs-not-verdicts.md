...
5. **Simulated accessibility results are one-sided, by the same rule as clause 3.** A simulated
   ...
   **success is not** evidence that a person would succeed, and may not be reported as an
   ...
   rule is fixed in EXP-75 before collection and only EXP-77 can lift it.
...
| EXP-74, EXP-75, EXP-76 | **PRODUCT** | measure the shipped harness |
...
| EXP-77's recruitment, consent and payment | **INSTANCE input to a PRODUCT measurement** | Joe-only under ADR-0033 |
...
- `[measured]` `cli.py` alone yielded **1,104 mutants, 440 composite survivors, 400 classified
  ...
  bad-artefact population EXP-74 needs already exists, is mechanically labelled, and is confirmed
  *not caught* by the existing checks. This is what lets EXP-74 run without the human-labelled
...
- `[cited]` Ratchet, arXiv:2605.22148v3 — the only prior-art self-extending system that reports
  ...
  EXP-74 measures α_sim for that reason and not for symmetry.
...
- `[asserted]` Five as the size of the concept budget. It is a defensible list, not a derived
  number, and EXP-75 and EXP-76 exist because of that.
...
- **Ratchet's false-fail rate is the likeliest killer, and it is not β.** ≈0.95 `[cited]`. A driven
  ...
  rejecting nearly everything good. EXP-74's α_sim may be the number that ends this, and it would
  end it for a reason clause 3 does not protect against.
- **Mutants are not real defects.** EXP-74's population is mechanically labelled, which is its
  ...
  detection — exactly the dissociation that would make EXP-74's result fail to transfer. That
...
- **The one-sided rule is suspiciously convenient, and this is the weakest point in the ADR.** It
  ...
  method can never be shown to over-claim. EXP-77 is the only thing that repairs this and it is
...
- **The concept budget is my judgement with no evidence behind the number five.** I have not cited
  ...
  hidden list could be hiding something load-bearing; EXP-76's dangerous cell is the check on
  exactly that.
...
stops being a slogan and becomes seven tasks with artefact-checkable outcomes. EXP-74 becomes
...
- **EXP-74 stopping rule 2 fires** — the driven session's marginal yield has a Wilson upper bound
...
- **EXP-74 stopping rule 4 fires** — arm C's findings are ≥50% state-anchored, or within 10 points
...
- **EXP-74 returns α_sim near Ratchet's 0.95.** Admissible in principle, unaffordable in practice;
  the conjunct is removed and only implicit-oracle-gated reporting survives.
- **EXP-75 shows arm B completing ≥6 of 7 tasks at ≥80%.** The current documentation is not the
...
- **EXP-76's dangerous cell exceeds 10%.** Collapsing five tags to two loses information people act
  on. The tags go back to the surface and the concept budget is wrong.
- **EXP-77 shows P(person fails | simulation failed) < 0.5.** Simulated failure does not transfer
...
**No**, per the default. One thing could change that: if EXP-74 returns a β_sim and an α_sim for a
...
V0-23 · EXP-01, EXP-13, EXP-47, EXP-49, **EXP-74, EXP-75, EXP-76, EXP-77** ·
